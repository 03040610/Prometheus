document.addEventListener('DOMContentLoaded', function(){
 ...
 const previewList = document.getElementById('previews');
  // 顔写真ページのフォームがないならここで終了。「!」は論理否定演算子。
  if (!postForm) return null;

  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="teacher[face_image]"]');
...
document.addEventListener('DOMContentLoaded', function(){
  // 新規投稿・編集ページのフォームを取得
  const postForm = document.getElementById('new_session');
  // プレビューを表示するためのスペースを取得
  const previewList = document.getElementById('previews');
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'teacher[certificate_images][]');

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="teacher[certificate_images][]"]:last-child');
  ...
  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="teacher[certificate_images][]"]');
